...
function GroupSearcher({
	...
	const [appliedFiltersRowStructure, setAppliedFiltersRowStructure] = useState([
		CLEARED_STATE_FILTER,
	]);
	...
	useEffect(
		() => groupToDelete && openDeleteGroupConfirmDialog(),
		[groupToDelete]
	);
	...
	const headers = () => {
		const headers = ['group.code', 'group.head'];

		headers.push(
			...Array.from(
				{ length: LOC_LEVELS },
				(_, i) => `location.locationType.${i}`
			)
		);
	...
	const itemFormatters = () => {
		...
		const locations = Array.from(
			{ length: LOC_LEVELS },
			(_, i) => (group) => locationAtLevel(group.location, LOC_LEVELS - i - 1)
		);
		...
		if (rights.includes(RIGHT_GROUP_DELETE) && isModalEnrollment === false) {
			formatters.push((group) => (
				<Tooltip
					title={formatMessage(intl, 'individual', 'deleteButtonTooltip')}
				>
					<IconButton
	...
	useEffect(() => {
		if (groupExport) {
			downloadExport(
				groupExport,
				`${formatMessage(intl, 'individual', 'export.filename.groups')}.csv`
			)();
			clearGroupExport();
		}
		...
		<div>
			<Searcher
				...
				{...(isModalEnrollment === false
					? {
							actionsContributionKey: INDIVIDUAL_GROUP_MENU_CONTRIBUTION_KEY,
...
export default withHistory(
	withModulesManager(
		injectIntl(connect(mapStateToProps, mapDispatchToProps)(GroupSearcher))
	)
);
