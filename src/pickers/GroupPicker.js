...
function GroupPicker(props) {
	...
	const fetchingGroups = useSelector(
		(state) => state.individual.fetchingGroups
	);
	...
	const getGroupsWithoutCurrentGroup = (options) =>
		options.filter((option) => option?.code !== groupIndividual?.group?.code);
	...
	return (
		<Autocomplete
			...
			onInputChange={(search) => {
				if (search !== undefined)
					setFilters([`code_Icontains: "${search}"`, 'isDeleted: false']);
			}}
		/>
	);
}
