...
function IndividualAddToGroupPicker(props) {
    ...
    `,
		...
		const shouldShowTooltip =
			individuals?.length >= INDIVIDUALS_QUANTITY_LIMIT &&
			!value &&
			!currentString;
		...
		return (
			<Autocomplete
				...
				onChange={(value) =>
					onChange(
						value,
						value ? `${value.firstName} ${value.lastName} ${value.dob}` : null
					)
				}
				...
				renderInput={(inputProps) => (
					<Tooltip
						title={
							shouldShowTooltip
								? formatMessageWithValues('IndividualsPicker.aboveLimit', {
										limit: INDIVIDUALS_QUANTITY_LIMIT,
								  })
								: ''
						}
					>
						<TextField
							...
							label={
								...
								formatMessage('Individual')
							}
							placeholder={
								(withPlaceholder && placeholder) ||
								formatMessage('IndividualPicker.placeholder')
							}
						/>
					</Tooltip>
				)}
	...
	const individuals =
		data?.individual?.edges.map((edge) => {
			const { node } = edge;
			return {
				...node,
			...
			renderInput={(inputProps) => (
				...
				>
					<TextField
						...
						required={required}
						label={
							(withLabel && (label || nullLabel)) || formatMessage('Individual')
						}
						placeholder={
							(withPlaceholder && placeholder) ||
							formatMessage('IndividualPicker.placeholder')
